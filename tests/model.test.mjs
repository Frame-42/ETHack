...
const company = (ticker, a = {}) => ({
  ticker,
  tickers: [ticker],
  cik: ticker,
  name: ticker,
  sector: "Test",
  assessment: { ctt: 1, tpq: 3, nature: 40, social: 50, ...a },
});
...
test("unknown is distinct from zero and cannot win a rank", () => {
  ...
  assert.deepEqual(
    [score(unknown).value, score(unknown).lower, score(unknown).upper],
    [null, 0, 100],
  );
  const rows = rankCompanies([unknown, zero]);
  assert.equal(rows[0].ticker, "Zero");
...
test("completion bounds contain every possible missing nature value", () => {
  ...
  assert.ok(
    score(c).complete,
    "Nature is not required by the explicitly named core lens",
  );
  assert.equal(
    score(c, { ...EXTENDED_WEIGHTS, nature: 0 }, 50, "extended").value,
    null,
    "Zeroing a weight does not change eligibility",
  );
});
test("weights normalize; invalid weights and ordinal mapping fail loudly", () => {
  ...
  for (const x of [0, -1, NaN, Infinity])
    assert.throws(() =>
      normalizedWeights({ contribution: x, planning: 0, nature: 0, social: 0 }),
    );
...
test("ties use competition ranks and increasing any pillar cannot lower the score", () => {
  ...
  for (const [key, next] of [
    ...
    assert.ok(
      score(company("X", { [key]: next })).value >= score(company("X")).value,
    );
});
...
test("sensitivity keeps incomplete companies unranked and evaluates 15 scenarios", () => {
  ...
  assert.deepEqual(s.A, { best: 1, worst: 1, scenarios: 15 });
  assert.equal(s.B, null);
});
test("sector percentiles suppress tiny samples and handle ties at the midpoint", () => {
  const small = rankCompanies([company("A"), company("B")]);
  assert.equal(peerPercentile(small[0], small), null);
  const tied = rankCompanies(["A", "B", "C", "D", "E"].map((t) => company(t)));
  assert.equal(peerPercentile(tied[0], tied).value, 50);
});
test("CSV preserves source absence and escapes embedded quotes", () => {
  const rows = rankCompanies([
    { ...company("A"), name: 'A, "quoted" company' },
    { ticker: "B", tickers: ["B"], name: "B", cik: "B" },
  ]);
  const csv = csvText(rows);
  assert.match(csv, /"A, ""quoted"" company"/);
