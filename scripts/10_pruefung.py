...
REPARATUREN = [
    ...
    ("Join vervielfacht", "TSLA", "1 Anlage, Historie SSSSSSSSSSSS, 2 Join-Zeilen",
     lambda l: (max(_werte(l, "TSLA", "echo_nc_quarters"), default=0)
                <= 12 * max(_werte(l, "TSLA", "echo_facilities"), default=1),
                f"{max(_werte(l, 'TSLA', 'echo_nc_quarters'), default=0):.0f} Quartale auf "
                f"{max(_werte(l, 'TSLA', 'echo_facilities'), default=0):.0f} Anlage statt 24")),
    ("Kein gerechnetes Verhaeltnis", "GE", "4,44 t/MWh war unsere Rate, nicht die von eGRID",
     lambda l: (not _werte(l, "GE", "t_co2_pro_mwh") and "t_co2_pro_mwh" not in set(l.metric),
                "keine Rate im Bestand, nur gemeldete Menge und Erzeugung")),
    ("Zaehler und Nenner bleiben", "BRK-B", "113 Kraftwerke, 83 TWh -- GICS sagt Financials",
     lambda l: (bool(_werte(l, "BRK-B", "egrid_mwh")) and bool(_werte(l, "BRK-B", "egrid_co2_t")),
                f"{max(_werte(l, 'BRK-B', 'egrid_mwh'), default=0)/1e6:.1f} TWh und "
                f"{max(_werte(l, 'BRK-B', 'egrid_co2_t'), default=0)/1e6:.1f} Mt gemeldet")),
    ("Keine Unfallrate aus ungepruefter Angabe", "MCD",
     "4 Meldungen mit im Median 50 h je Beschaeftigtem",
     lambda l: ("dart_rate" not in set(l.metric) and bool(_werte(l, "MCD", "osha_hours")),
                "gemeldete Stunden und Faelle statt gerechneter Rate")),
    ...
    ("Kein gerechneter Trend", "PPL", "2018 eine Anlage 5 kt, ab 2019 neun Anlagen 28 Mt",
     lambda l: ({"absolute_cagr", "intensity_cagr", "co2_intensity"} & set(l.metric) == set()
                and bool(_werte(l, "PPL", "n_facilities")),
                "keine Trend- oder Intensitaetswerte im Bestand, Anlagenzahl steht daneben")),
    ...
    ("Nur Werte mit Quelle", "alle", "jeder Wert ist gemeldet oder eine Summe gemeldeter Werte",
     lambda l: (set(l.wert_art.dropna()) <= {"gemeldet", "aggregiert"} and not l.wert_art.isna().any(),
                f"{l.wert_art.value_counts().to_dict()}")),
]
