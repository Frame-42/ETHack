...
const sectors = [...new Set(ranked.map((c) => c.sector))]
  .sort()
  .map((sector) => {
    ...
    return {
      sector,
      total: rows.length,
      matched: rows.filter((c) => c.wba).length,
      complete: rows.filter((c) => c.result.complete).length,
      extended: extended.filter((c) => c.sector === sector && c.result.complete)
        .length,
    };
  });
const metadata = {
  ...
  securities: raw.security_count,
  issuers: ranked.length,
  matched: ranked.filter((c) => c.wba).length,
  complete: ranked.filter((c) => c.result.complete).length,
  extended: extended.filter((c) => c.result.complete).length,
  directory_rows: raw.directory_rows_returned,
  directory_headline: raw.directory_headline_count,
  snapshot_sha256: createHash("sha256").update(bytes).digest("hex"),
  universe_source: raw.universe_source,
  sectors,
  sensitivity_scenarios: 15,
};
const companies = ranked.map(({ result, rank, ...c }) => ({
  ...c,
  baseline_rank: rank,
  sensitivity: { core: ranges[c.cik], extended: extendedRanges[c.cik] },
}));
await mkdir(new URL("data/processed/", root), { recursive: true });
await writeFile(
  new URL("data/processed/companies.json", root),
  JSON.stringify({ metadata, companies }, null, 2) + "\n",
);
await writeFile(
  new URL("data/processed/ranking.csv", root),
  csvText(ranked) + "\n",
);
await writeFile(
  new URL("data/processed/ranking-with-nature.csv", root),
  csvText(extended) + "\n",
);
const leaders = (rows, r) =>
  `| Company | Score | Rank across 15 assumptions |\n|---|---:|---:|\n${rows
    .filter((c) => c.result.complete)
    .slice(0, 12)
    .map(
      (c) =>
        `| [${c.name} (${c.ticker})](${c.assessment.source.url}) | ${c.result.value.toFixed(1)} | ${r[c.cik].best}–${r[c.cik].worst} |`,
    )
    .join("\n")}`;
const cttCounts = [0, 1, 2].map(
  (n) => ranked.filter((c) => c.assessment?.ctt === n).length,
);
...
console.log(
  `Built ${metadata.issuers} issuers; ${metadata.complete} core, ${metadata.extended} with nature. CTT: ${cttCounts.join("/")}`,
);
